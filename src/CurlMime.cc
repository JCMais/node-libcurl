...
CurlMime::CurlMime(const Napi::CallbackInfo& info)
  ...
  Easy* easy = nullptr;
  napi_status status = napi_unwrap(env, handleObj, reinterpret_cast<void**>(&easy));

  if (status == napi_ok && easy != nullptr && easy->ch != nullptr) {
    // It's an Easy instance
    ...
    if (constructorVal.IsObject()) {
      ...
      if (nameVal.IsString() && nameVal.As<Napi::String>().Utf8Value() == "Curl") {
        ...
        if (handleVal.IsObject()) {
          Napi::Object easyObj = handleVal.As<Napi::Object>();
          Easy* curlEasy = nullptr;
          status = napi_unwrap(env, easyObj, reinterpret_cast<void**>(&curlEasy));

          if (status == napi_ok && curlEasy != nullptr && curlEasy->ch != nullptr) {
...
//=============================================================================
// Phase 1: Basic MIME Part Methods
//=============================================================================
...
Napi::Value CurlMimePart::SetFiledata(const Napi::CallbackInfo& info) {
  ...
  if (!info[0].IsString()) {
    throw Napi::TypeError::New(env, "filepath must be a string");
  }
  ...
  if (code != CURLE_OK) {
    throw CurlError::New(env, "Failed to set MIME part file data", code);
  }
...
Napi::Value CurlMimePart::SetType(const Napi::CallbackInfo& info) {
  ...
  if (!info[0].IsString()) {
    throw Napi::TypeError::New(env, "mimetype must be a string");
  }
  ...
  if (code != CURLE_OK) {
    throw CurlError::New(env, "Failed to set MIME part type", code);
  }
...
Napi::Value CurlMimePart::SetFilename(const Napi::CallbackInfo& info) {
  ...
  if (!info[0].IsString()) {
    throw Napi::TypeError::New(env, "filename must be a string");
  }
  ...
  if (code != CURLE_OK) {
    throw CurlError::New(env, "Failed to set MIME part filename", code);
  }
...
//=============================================================================
// Phase 2: Advanced MIME Part Methods
//=============================================================================
...
Napi::Value CurlMimePart::SetSubparts(const Napi::CallbackInfo& info) {
  ...
  CurlMime* subMime = nullptr;
  napi_status status = napi_unwrap(env, mimeObj, reinterpret_cast<void**>(&subMime));

  if (status != napi_ok || subMime == nullptr || subMime->mime == nullptr) {
    throw Napi::TypeError::New(env, "Argument must be a valid CurlMime instance");
  }

  // Set subparts - ownership transfers to this part
