...
const imageFilePath = path.resolve(__dirname, 'upload.png')
...
it('should upload file correctly', done => {
  ...
  curl.on('end', (status, data) => {
    ...
    data.name.should.be.equal(imageFilePath.split(path.sep).pop())
...
it('should upload file correctly with filename', done => {
  ...
  curl.on('end', (status, data) => {
    ...
    parsedData.name.should.be.equal(name)
    parsedData.type.should.be.equal(postData[0].type)
