/**
 * Copyright (c) Jonathan Cardoso Machado. All Rights Reserved.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
/**
 * yay this has code from the httppost and postfields test cases.
 * This was done because the duplicated handle shares the ToFree instances of the original handle,
 *  so we need to test their behavior when a handle is duplicated.
 * We can test that easily by testing HTTPPOST and POSTFIELDS.
 */
import {
  describe,
  beforeEach,
  afterEach,
  beforeAll,
  afterAll,
  it,
  expect,
} from 'vitest'

import fs from 'fs/promises'
import path from 'path'
import crypto from 'crypto'

import querystring from 'querystring'
import formidable from 'formidable'

import { createServer } from '../helper/server'
import { Curl } from '../../lib'
import assert from 'assert'
import { withCommonTestOptions } from '../helper/commonOptions'

const image =
  ''
const imageFilename = path.resolve(
  __dirname,
  `${crypto.randomBytes(8).toString('hex')}.png`,
)

const buffer = Buffer.from(image, 'base64')
const size = buffer.length

const httpPostData = [
  {
    name: 'file',
    file: imageFilename,
    type: 'image/png',
  },
]
const postFieldsData: { [key: string]: string } = {
  'input-name': 'This is input-name value.',
  'input-name2': 'This is input-name2 value',
}

let curl: Curl
let duplicatedCurl: Curl
let serverInstance: ReturnType<typeof createServer>

describe('dupHandle()', () => {
  beforeEach(() => {
    curl = new Curl()
    withCommonTestOptions(curl)
  })

  afterEach(() => {
    curl && curl.close()
    duplicatedCurl && duplicatedCurl.close()
  })

  beforeAll(async () => {
    serverInstance = createServer()

    serverInstance.app.post('/urlencoded', (req, res) => {
      res.send(JSON.stringify(req.body))
    })

    serverInstance.app.post('/multipart', (req, res, next) => {
      // parse a file upload
      // @ts-ignore
      const form = formidable({ multiples: true })

      // @ts-ignore
      form.parse(req, (error, _fields, files) => {
        if (error) {
          next(error)
          return
        }

        const file = files.file?.[0]
        assert(file)

        const response = {
          size: file.size,
          name: file.originalFilename,
          type: file.mimetype,
        }

        res.json(response)
      })
    })

    await serverInstance.listen()
    await fs.writeFile(imageFilename, buffer)
  })

  afterAll(async () => {
    await fs.unlink(imageFilename)
    await serverInstance.close()
    serverInstance.app._router.stack.pop()
    serverInstance.app._router.stack.pop()
  })

  it('should correctly send HTTPPOST data when duplicated', async () => {
    curl.setOpt('URL', `${serverInstance.url}/multipart`)
    curl.setOpt('HTTPPOST', httpPostData)

    duplicatedCurl = curl.dupHandle()
    // close original curl to check if data is removed
    curl.close()
    curl = new Curl()

    const result = await new Promise<{ status: number; data: string }>(
      (resolve, reject) => {
        duplicatedCurl.on('end', (status, data) => {
          if (status !== 200) {
            reject(new Error(`Invalid status code: ${status}`))
            return
          }

          resolve({ status, data: data as string })
        })

        duplicatedCurl.on('error', reject)

        duplicatedCurl.perform()
      },
    )

    const parsedData = JSON.parse(result.data)

    expect(parsedData.size).toBe(size)
    expect(parsedData.name).toBe(imageFilename.split(path.sep).pop())
    expect(parsedData.type).toBe(httpPostData[0].type)
  })

  it('should correctly send POSTFIELDS data when duplicated', async () => {
    curl.setOpt('URL', `${serverInstance.url}/urlencoded`)
    curl.setOpt('POSTFIELDS', querystring.stringify(postFieldsData))

    duplicatedCurl = curl.dupHandle()
    // close original curl to check if data is removed
    curl.close()
    curl = new Curl()

    const result = await new Promise<{ status: number; data: string }>(
      (resolve, reject) => {
        duplicatedCurl.on('end', (status, data) => {
          if (status !== 200) {
            reject(new Error(`Invalid status code: ${status}`))
            return
          }

          resolve({ status, data: data as string })
        })

        duplicatedCurl.on('error', reject)

        duplicatedCurl.perform()
      },
    )

    const parsedData = JSON.parse(result.data)

    for (const field in parsedData) {
      if (Object.prototype.hasOwnProperty.call(parsedData, field)) {
        expect(parsedData[field]).toBe(postFieldsData[field])
      }
    }
  })
})
