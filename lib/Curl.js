...
class Curl extends EventEmitter {
  ...
  _onData(chunk, size, nmemb) {
     ...
     * @param {module:node-libcurl.Curl} curl Handle that emitted this event, `this` will also point to it as normal
     */
  ...
  _onError(err, errCode) {
     ...
     * @param {module:node-libcurl.Curl} curl Handle that emitted this event, `this` will also point to it as normal
     */
    this.emit('error', err, errCode, this)
  }
  ...
  _onEnd() {
    const isHeaderStorageEnabled = !(
      this._features & features.NO_HEADER_STORAGE
    )
    const isDataStorageEnabled = !(this._features & features.NO_DATA_STORAGE)
    const isHeaderParsingEnabled =
      !(this._features & features.NO_HEADER_PARSING) && isHeaderStorageEnabled
    const isDataParsingEnabled =
      !(this._features & features.NO_DATA_PARSING) && isDataStorageEnabled

    this._isRunning = false

    const data = isDataStorageEnabled
      ? _mergeChunks(this._chunks, this._chunksLength)
      : Buffer.alloc(0)
    const header = isHeaderStorageEnabled
      ? _mergeChunks(this._headerChunks, this._headerChunksLength)
      : Buffer.alloc(0)
    ...
    const argHeader = isHeaderParsingEnabled
      ? _parseHeaders(decoder.write(header))
      : header
     ...
     * @param {Array|Buffer} argHeader If [Curl.feature.NO_HEADER_PARSING]{@link module:node-libcurl.Curl.feature} is set, a Buffer is passed instead of an array with the headers.
     * @param {module:node-libcurl.Curl} curl Handle that emitted this event, `this` will also point to it as normal
     */
    this.emit('end', status, argBody, argHeader, this)
  }
   ...
   */
  dupHandle(shouldCopyCallbacks, shouldCopyEventListeners) {
    shouldCopyCallbacks =
      typeof shouldCopyCallbacks === 'undefined' ? true : shouldCopyCallbacks
    shouldCopyEventListeners =
      typeof shouldCopyEventListeners === 'undefined'
        ? true
        : shouldCopyEventListeners
